# Docker Flow: Proxy - Swarm Mode (Docker 1.12+) With Automatic Configuration On Ubuntu 16.04 With VirtualBox And Vagrant
...
vagrant up swarm-manager-1 swarm-worker-1 swarm-worker-2
```
...
We can see the cluster status by entering the manager VM and running the following command.

```bash
vagrant ssh swarm-manager-1
...
```bash
curl -i localhost/something/hello
```
...
```bash
curl -i "https://10.100.192.200/demo/hello"
```

The output is as follows.

```
curl: (35) Unknown SSL protocol error in connection to 10.100.192.200:-9847
```
...
```bash
curl -i http://10.100.192.200.xip.io/demo/hello
```
...
curl -i -XPUT \
    ...
    "10.100.192.200:8080/v1/docker-flow-proxy/cert?certName=xip.io.pem&distribute=true"
```
...
```bash
curl 10.100.192.200.xip.io:8080/v1/docker-flow-proxy/config
```
...
```bash
curl -i  \
    "https://10.100.192.200.xip.io/demo/hello"
```
...
```bash
echo "https://10.100.192.200.xip.io/demo/hello"
```
