...
|CFG_TEMPLATE_PATH  |Path to the configuration template. The path can be absolute (starting with `/`) or relative to `/cfg/tmpl`|/cfg/tmpl/haproxy.tmpl|
...
|DEBUG_ERRORS_ONLY  |If set to `true`, only requests that resulted in an error, timeout, retry, and redispatch will be logged. If a request is HTTP, responses with a status 5xx will be logged too. This variable will take effect only if `DEBUG` is set to `true`.<br>Example: true|false|
...
|DEFAULT_PORTS      |The default ports used by the proxy. Multiple values can be separated with comma (`,`). If a port should be for SSL connections, append it with `:ssl.|80,443:ssl|
...
|MODE               |Two modes are supported. The *default* mode should be used for general purpose. **This mode is deprecated and will be removed soon**. The *swarm* mode is designed to work with new features introduced in Docker 1.12 and assumes that containers are deployed as Docker services (new Swarm).<br>Example: swarm|default|
PROXY_INSTANCE_NAME|The name of the proxy instance. Useful if multiple proxies are running inside a cluster.|docker-flow|
|SERVICE_NAME       |The name of the service. It must be the same as the value of the `--name` argument used to create the proxy service. Used only in the *swarm* mode.<br>Example: my-proxy|proxy|
|SKIP_ADDRESS_VALIDATION|Whether to skip validating service address before reconfiguring the proxy.<br>Example: false|true|
|SSL_BIND_CIPHERS   |Sets the default string describing the list of cipher algorithms ("cipher suite") that are negotiated during the SSL/TLS handshake for all "bind" lines which do not explicitly define theirs. The format of the string is defined in "man 1 ciphers" from OpenSSL man pages, and can be for instance a string such as `AES:ALL:!aNULL:!eNULL:+RC4:@STRENGTH`.|[Dockerfile](https://github.com/vfarcic/docker-flow-proxy/blob/master/Dockerfile#L28)|
|SSL_BIND_OPTIONS   |Sets default ssl-options to force on all "bind" lines.   |no-sslv3|
|STATS_USER         |Username for the statistics page. If not set, stats will not be available. If both `STATS_USER` and `STATS_PASS` are set to `none`, statistics will be available without authentication.<br>Example: my-user|admin  |
...
|STATS_PASS         |Password for the statistics page. If not set, stats will not be available. If both `STATS_USER` and `STATS_PASS` are set to `none`, statistics will be available without authentication.<br>Example: my-pass|admin|
...
|STATS_URI          |URI for the statistics page.<br>Example: proxyStats       |/admin?proxy|
...
|TIMEOUT_CLIENT     |The client timeout in seconds.<br>Example: 5              |20     |
|TIMEOUT_CONNECT    |The connect timeout in seconds.<br>Example: 3             |5      |
|TIMEOUT_QUEUE      |The queue timeout in seconds.<br>Example: 10              |30     |
|TIMEOUT_SERVER     |The server timeout in seconds.<br>Example: 15             |20     |
|TIMEOUT_TUNNEL     |The tunnel timeout in seconds.<br>Example: 1800           |3600   |
|TIMEOUT_HTTP_REQUEST|The HTTP request timeout in seconds.<br>Example: 3       |5      |
|TIMEOUT_HTTP_KEEP_ALIVE|The HTTP keep alive timeout in seconds.<br>Example: 10|15     |
...
|USERS_PASS_ENCRYPTED| Indicates if passwords provided through USERS or Docker secret `dfp_users` (`/run/secrets/dfp_users` file) are encrypted. Passwords can be encrypted with the `mkpasswd -m sha-512 my-password` command.<br>Example: true|false|
