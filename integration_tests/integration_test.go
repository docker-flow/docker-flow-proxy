...
//
//	testAddr := "http://staging-service-proxy/v1/test"
...
//	address := fmt.Sprintf(
//		"http://staging-service-proxy:8080/v1/docker-flow-proxy/reconfigure?serviceName=%s&servicePath=%s&users=%s",
//		s.serviceName,
//		"/v1/test",
...
////	address := fmt.Sprintf(
////		"http://staging-service-proxy:8080/v1/docker-flow-proxy/reconfigure?serviceName=%s&servicePath=%s&port=1111&servicePath.1=%s&port.1=2222",
////		s.serviceName,
////		"/v1/test",
////		"/v1/test",
////	)
...
//	urlObj, _ := url.Parse(
//		"http://staging-service-proxy:8080/v1/docker-flow-proxy/reconfigure",
//	)
...
//	// Returns status 200
//
//	testAddr := "http://staging-service-proxy/v99/test"
//	log.Printf(">> Sending verify request to %s", testAddr)
//	client := &http.Client{}
//	request, _ := http.NewRequest("GET", testAddr, nil)
//	request.SetBasicAuth("user1", "pass1")
...
//	// Returns status 401 if no auth is provided
//
//	testAddr := "http://staging-service-proxy/admin?stats"
//	log.Printf(">> Sending verify request to %s", testAddr)
//	client := &http.Client{}
//	request, _ := http.NewRequest("GET", testAddr, nil)
//	resp, err := client.Do(request)
...
//	aclName := "my-acl"
//	address := fmt.Sprintf(
//		"http://staging-service-proxy:8080/v1/docker-flow-proxy/reconfigure?serviceName=%s&servicePath=%s&aclName=%s",
//		s.serviceName,
//		"/v1/test",
//		aclName,
//	)
...
//	s.verifyReconfigure(1)
//
//	url := "http://staging-service-proxy:8080/v1/docker-flow-proxy/remove?serviceName=test-service"
//	log.Printf(">> Sending remove request to %s", url)
//	_, err = http.Get(url)
//
//	s.NoError(err)
//	url = "http://staging-service-proxy/v1/test"
...
//
//	url := fmt.Sprintf(
//		"http://staging-service-proxy:8500/v1/kv/proxy-test-instance/%s/path?raw",
//		s.serviceName,
//	)
//	resp, _ := http.Get(url)
...
//	http.Get(
//		"http://staging-service-proxy:8080/v1/docker-flow-proxy/remove?serviceName=test-service",
//	)
...
//	resp, _ := http.Get("http://staging-service-proxy:8080/v1/docker-flow-proxy/config")
...
//	// Body is mandatory
//	url := "http://staging-service-proxy:8080/v1/docker-flow-proxy/cert?certName=xip.io.pem"
...
//	url = "http://staging-service-proxy:8080/v1/docker-flow-proxy/cert"
...
//
//	// Can retrieve certs
//	url = "http://staging-service-proxy:8080/v1/docker-flow-proxy/certs"
...
func (s IntegrationTestSuite) verifyReconfigure(version int) {
	address := fmt.Sprintf("http://staging-service-proxy/v%d/test", version)
...
func (s IntegrationTestSuite) reconfigure(pathType, consulTemplateFePath, consulTemplateBePath string, paths ...string) {
	...
	if len(consulTemplateFePath) > 0 {
		address = fmt.Sprintf(
			"http://staging-service-proxy:8080/v1/docker-flow-proxy/reconfigure?serviceName=%s&consulTemplateFePath=%s&consulTemplateBePath=%s",
			s.serviceName,
		...
		address = fmt.Sprintf(
			"http://staging-service-proxy:8080/v1/docker-flow-proxy/reconfigure?serviceName=%s&servicePath=%s&pathType=%s",
			s.serviceName,
			strings.Join(paths, ","),
			pathType,
		)
...
func (s IntegrationTestSuite) getConf() string {
	configAddr := "http://staging-service-proxy:8080/v1/docker-flow-proxy/config"
